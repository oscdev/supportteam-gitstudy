...
class Ophirah_Core_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        $icLoaderVersion = false;

        if (function_exists('ioncube_loader_iversion')) {
            $icLoaderIversion = ioncube_loader_iversion();
            $icLoaderVersionMajor = (int)substr($icLoaderIversion, 0, 1);
            $icLoaderVersionMinor = (int)substr($icLoaderIversion, 1, 2);
            $icLoaderVersionRevision = (int)substr($icLoaderIversion, 3, 2);
            $icLoaderVersion = "$icLoaderVersionMajor.$icLoaderVersionMinor.$icLoaderVersionRevision";
        } else {
            if (function_exists('ioncube_loader_version')) {
                $icLoaderVersion = ioncube_loader_version();
            }
        }

        return $icLoaderVersion;
    }
