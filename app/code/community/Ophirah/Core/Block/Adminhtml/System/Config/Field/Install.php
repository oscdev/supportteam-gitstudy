...
class Ophirah_Core_Block_Adminhtml_System_Config_Field_Install extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        if ($this->helper->checkCache()) {
            return $this->__('Cache is enebaled, please disable the cache in "System>Cache Management" before updating or installing Cart2Quote');
        }

        //check if the compiler is enabed
        if ($this->helper->checkCompiler()) {
            return $this->__('The compiler is enebaled, please disable the compiler in "System>Tools>Compilation" before updating or installing Cart2Quote');
        }
        ...
        if ($this->versionModel->getInstallType() == Ophirah_Core_Model_Version::INSTALL_TYPE_EC) {
            ...
            if ($ioncubeVersion == false) {
                ...
                if ((version_compare($phpVersion, '7.0.0') >= 0) && (version_compare($ioncubeVersion, '6.0.2') < 0)) {
                    return $this->__('The current version of ionCube for PHP7 is not viable for a production store, please install ionCube loader 6.0.2 or newer. Alternatively you could use the opensource version of Cart2Quote');
                }
            }
        }
