...
class Ophirah_Qquoteadv_Model_Pdf_Qquote extends Mage_Sales_Model_Order_Pdf_Abstract
{
    ...
    public function getPdf($quotes = array())
    {
        ...
        foreach ($requestItems as $product) {
            ...
            /* Draw item */
            if(Mage::helper('core')->isModuleEnabled('Ophirah_Fakepro') && Mage::helper('fakepro')->isFakeProduct($product->getProductId())){
                $page = Mage::getModel('fakepro/pdf_product')->setQuote($this->_quoteadv)->setPdfModel($this)->draw($product, $page);
            }else{
...
//                    $html[] = '  ' . $_formatedOptionValue['value'];
    ...
    protected function insertTitles(&$page, $source, $storeId)
    {
        ...
        if($countShippingAddress < 3){
            $countShippingAddress = 3;
        }

        $addressHeight = ($countShippingAddress * $this->fontLineHeightDefault) + 2;
...
//        $proposalDate = $this->_quoteadv->getProposalSent();  //Mage::helper('core')->formatDate( date( 'D M j Y')
        ...
        if ($expiryDate) {
            // PHP > 5.2 && PHP < 5.3
            $expDays = (int)round((date_create($expiryDate)->format("U") - date_create($proposalDate)->format("U")) / 86400);
            // PHP >= 5.3
//            $expDays    = (int) date_diff(date_create($proposalDate), date_create($expiryDate))->format('%a');
        ...
        if ($expDays && $validDate) {
            ...
            if($expDays > 0){
                ...
                $value = Mage::helper('core')->formatDate($validDate, Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM, false);
            }
