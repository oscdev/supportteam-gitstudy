...
class Ophirah_Qquoteadv_Model_Qqadvproduct extends Mage_Core_Model_Abstract
{
    ...
    public function getConfChildProduct($productQuoteId)
    {
        ...
        if($childProduct === null){
            //Mage::helper('qquoteadv')->__("No product found for the given attributes");
    ...
    public function removeDefaultTax($price, $quoteCustomPrice){
        ...
        /*
        $productId = $quoteCustomPrice->getProductId();
        $quoteId = $quoteCustomPrice->getQuoteId();
        $storeId = Mage::getModel('qquoteadv/qqadvcustomer')->load($quoteId)->getStoreId();
        if (Mage::helper('tax')->priceIncludesTax($storeId)){
            ...
            if($product->getTypeId() != 'bundle'){
                ...
                if($priceType != 0){
                    ...
                    //dynamic bundle doesn't has a tax class
                    ...
                    foreach ($attribute['bundle_option'] as $key => $bindeOptionIdA) {
                        ...
                        foreach ($bindeOptionIds as $bindeOptionId) {
                            $childId = Mage::getModel('bundle/selection')->load($bindeOptionId)->getData('product_id');
                            $childProd = Mage::getModel('catalog/product')->load($childId);
                            $taxClassId = $childProd->getTaxClassId();
                            $percent = $taxCalculation->getRate($request->setProductClassId($taxClassId));
                            break;
    ...
    public function getCustomPriceCheck($quoteCustomPrices, $item, $optionCount = null)
    {
        ...
        foreach ($quoteCustomPrices as $requestId => $quoteCustomPrice) {
            ...
            if (isset($product_id) && $product_id == $quoteCustomPrice->getData('product_id')) {
                ...
                if (isset($buyRequest['bundle_option'])) {
                    ...
                    if ($item->getData('product_type') == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE) {
                        ...
                        $price = $this->removeDefaultTax($price, $quoteCustomPrice);
                        $cost = $this->removeDefaultTax($cost, $quoteCustomPrice);
                        ...
                        $item->setData('custom_base_price', $this->removeDefaultTax($customBasePrice[$quoteCustomPrice->getData('qty')], $quoteCustomPrice));
                        $item->setData('custom_cur_price', $this->removeDefaultTax($customCurPrice[$quoteCustomPrice->getData('qty')], $quoteCustomPrice));
            ...
            //check for dowloadable item and if so, check if the links in the items also exits in the custom price
            if (Mage::getModel('qquoteadv/qqadvproductdownloadable')->isDownloadable($item)){
                $downloadableCheck = Mage::getModel('qquoteadv/qqadvproductdownloadable')->exists($quoteCustomPrice, $item->getBuyRequest()->getLinks());
            } else {
                $downloadableCheck = false;
            }
            ...
            if($compareQuote == $compareItem){
                ...
                if(isset($customBasePrice[$quoteCustomPrice->getData('qty')]) && !empty($customBasePrice[$quoteCustomPrice->getData('qty')])){
                    ...
                    $customBasePriceExcludingTax = $this->removeDefaultTax($customBasePriceExcludingTax, $quoteCustomPrice);
                ...
                if(isset($customCurPrice[$quoteCustomPrice->getData('qty')]) && !empty($customCurPrice[$quoteCustomPrice->getData('qty')])){
                    ...
                    $customCurPriceExcludingTax = $this->removeDefaultTax($customCurPriceExcludingTax, $quoteCustomPrice);
            ...
            if ($compareQuote == $compareItem && !$downloadableCheck) {
                //set item cost
                if(isset($itemCost)){
                    $itemCost = $this->removeDefaultTax($itemCost, $quoteCustomPrice);
                    $item->setData('quote_item_cost', $itemCost);
                }
            } elseif ($compareQuote == $compareItem && $downloadableCheck) {
                //set item cost
                if(isset($itemCost)){
