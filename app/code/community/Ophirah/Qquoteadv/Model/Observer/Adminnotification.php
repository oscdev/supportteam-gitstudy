...
class Ophirah_Qquoteadv_Model_Observer_Adminnotification
{
    ...
    public function getPersonalQuoteCountMessage(){
        ...
        $personalQuotesCount = $personalQuoteCollection->count();

        if ($personalQuotesCount > 1) {
            $link = Mage::helper('adminhtml')->getUrl('adminhtml/qquoteadv');
            $personalMessage = Mage::helper('qquoteadv')->__("You have <a href='%s'> %s unanswered</a> quotation requests that require your action.", $link, $personalQuotesCount);
        } elseif ($personalQuotesCount == 1) {
            $link = Mage::helper('adminhtml')->getUrl('adminhtml/qquoteadv/edit', array('id' => $personalQuoteCollection->getFirstItem()->getId()));
            $personalMessage = Mage::helper('qquoteadv')->__("You have <a href='%s'> %s unanswered</a> quote request that require your action.", $link, $personalQuotesCount);
        }
    ...
    public function getTotalQuoteCountMessage(){
        ...
        $unansweredQuoteCount = $unansweredQuoteCollection->count();

        if( $unansweredQuoteCount > 1) {
            $link = Mage::helper('adminhtml')->getUrl('adminhtml/qquoteadv');
            $totalMessage = Mage::helper('qquoteadv')->__("(<a href='%s'>%s unanswered requests</a> in total).", $link, $unansweredQuoteCount);
        } elseif ($unansweredQuoteCount == 1){
            $link = Mage::helper('adminhtml')->getUrl('adminhtml/qquoteadv/edit', array('id' => $unansweredQuoteCollection->getFirstItem()->getId()));
            $totalMessage = Mage::helper('qquoteadv')->__("(<a href='%s'>%s unanswered request</a> in total).", $link, $unansweredQuoteCount);
        }
        return $totalMessage;
    }
    ...
    public function getNewQuoteCountMessage(){
        ...
        $newQuotesCount = $newQuoteCollection->count();

        if ($newQuotesCount > 1) {
            $link = Mage::helper('adminhtml')->getUrl('adminhtml/qquoteadv');
            $newMessage = Mage::helper('qquoteadv')->__("There are <a href='%s'>%s new</a> requests since your last login.", $link, $newQuotesCount);
        } elseif($newQuotesCount == 1){
            $link = Mage::helper('adminhtml')->getUrl('adminhtml/qquoteadv/edit', array('id' => $newQuoteCollection->getFirstItem()->getId()));
            $newMessage = Mage::helper('qquoteadv')->__("There is <a href='%s'>%s new</a> request since your last login.", $link, $newQuotesCount);
        }
        return $newMessage;
    }
