...
class Ophirah_Qquoteadv_Model_System_Config_Source_Shipping_Typeselect
{
    ...
    /**
     * Function that only returns a shipping method code if the give carrier implements Mage_Shipping_Model_Carrier_Interface
     *
    ...
    private function getShippingCodes($shippingCarrier)
    {
        if ($shippingCarrier instanceof Mage_Shipping_Model_Carrier_Interface) {
            ...
            if (is_object($shippingCarrier)) {
                Mage::log('Message: Shipping method "' . get_class($shippingCarrier) . '" does not implement "Mage_Shipping_Model_Carrier_Interface"', null, 'c2q.log', true);
            } else {
                Mage::log('Message: Shipping method is not an object', null, 'c2q.log', true);
            }
        }
        return false;
    }
