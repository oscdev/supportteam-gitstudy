...
class Ophirah_Qquoteadv_Block_Form_Address extends Ophirah_Qquoteadv_Block_Qquoteaddress
{
    ...
    public function getListOfAddresses(){
        ...
        if($parent->isCustomerLoggedIn()) {
            ...
            foreach ($customer->getAddresses() as $address) {
                $selected = $this->_getRightSelectText($primaryAddressesIds, $address->getId());
     ...
     * @return string
     */
    private function _getRightSelectText($primaryAddressesIds, $addressId)
    {
        if (is_array($primaryAddressesIds)) {
            if ($primaryAddressesIds[0] == $primaryAddressesIds[1] && $primaryAddressesIds[0] == $addressId) {
                return $selected = 'selected';
            } elseif ($primaryAddressesIds[0] == $addressId && $this->getAddressType() == self::BILLING) {
                return $selected = 'selected';
            } elseif ($primaryAddressesIds[1] == $addressId && $this->getAddressType() == self::SHIPPING) {
                return $selected = 'selected';
            } else {
                return $selected = '0';
            }
